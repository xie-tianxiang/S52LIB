...
bool s52plib::CheckTextRectList(const wxRect &test_rect, S52_TextC *ptext) {
  ...
  //   }
  // }
  for (auto node = m_textObjList.begin(); node != m_textObjList.end(); ++node) {
  ...
  if (text) {
    ...
    if (m_bDeClutterText) {
      if (bwas_drawn) {
        ...
        // }
        for (auto node = m_textObjList.begin(); node != m_textObjList.end(); ++node) {
          S52_TextC *oc = *node;  // 相当于原来的 node->GetData()
...
// }
void s52plib::AdjustTextList(int dx, int dy, int screenw, int screenh) {
    return;  // 注意：原代码开头有 return，这里保留但实际应该删除
  
    wxRect rScreen(0, 0, screenw, screenh);
    
    // 使用迭代器遍历
    auto node = m_textObjList.begin();
